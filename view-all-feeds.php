...
if(isset($_POST['submitmessagetag'])){	
	$messagetagsetCondition = " and ( `message_tag` LIKE  '%".trim($_POST['searchkeyword'])."%' OR `message` LIKE '%".trim($_POST['searchkeyword'])."%' ) ";
}
extract($filter_data);
$messageDataAllcount = $adminObj->selectActiveMessageAndriodPaginationArray($tablenamemsg, $message_type, $language_type, $day, $month, $year, $status='all', $user_privacy_settings_sql, $startLimit='', $endLimit='', $othercondition=" `status` IN ('1','5') ".$categoryCondition." ".$messagetagsetCondition."  ",$orderby='order by broadcast_date DESC');
$_SESSION['messageData'] = $messageDataAllcount;
$totalMsg      = count($messageDataAllcount);
$startLimit    = 1;
$endLimit      = 10;
$messageData   = $adminObj->selectActiveMessageAndriodPaginationArray($tablenamemsg, $message_type, $language_type,$day,$month,$year, $status='all', $user_privacy_settings_sql, $startLimit, $endLimit , $othercondition=" `status` IN ('1','5') ".$categoryCondition." ".$messagetagsetCondition." ",$orderby='order by broadcast_date DESC');
//echo '<pre>';print_r($_SESSION['messageData']);echo '</pre>';
?>
...
<div class="container">
	...
	<div id="paginationcontent" class="entry">
		...
		<!-- <div id="content_loder_empty" class="txtcenter"><img src="/images/loading.gif" alt="Loading Please Wait..."></div> -->
	...
	<?php if(isset($totalMsg) && $totalMsg > 10){?>
		<!------- BANNER-START ----------->
		<?php require_once($DOC_ROOT.'ajax-pagination-array.php');?>
		<!------- BANNER-END ------------->	
	<?php } ?>

...
if(!empty($cdrvoiceData)){
	...
	$sqlordinates  = "SELECT mo.*,m.*,m.date as mdate FROM `message_ordinates_pa` mo JOIN `message_pa` m ON mo.msg_id = m.id WHERE DATE_FORMAT(mo.date,'%Y-%m-%d') = '".$sendDate."' and m.status='5' and `message_file` != '' ";
		...
		if(!empty($cdrMessageData)){
			foreach($cdrMessageData as $districtkey => $messageAll){
				foreach($messageAll as $phonekey => $message){
					...
					$sql = "INSERT into `message_pa` SET
			...
			if(!empty($messageidArray)){
				foreach($messageidArray as $msg_id){
					//UPDATING MSG STATUS
					$sql	= "UPDATE `message_pa` SET `status`='6' WHERE `id` = '".$msg_id."' ";
					$update = $db->update($sql);
				}
			}
			if(isset($insertResult) && $insertResult){
				echo 'Message inserted of date(Punjabi):'.$sendDate;exit;
			}
		}
	}else{
		echo 'No Message posted date(Punjabi): :'.$sendDate;exit;
	}
}else{
	echo 'No Message posted date(Punjabi): :'.$sendDate;exit;
}

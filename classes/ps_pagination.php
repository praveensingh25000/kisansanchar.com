...
class PS_Pagination
{
	...
	function renderNextAjax($tag='',$class = 'nextimage')
	{
		$root="http://maduhaa.com/";
		$URL_SITE = $root;
		$imgR='/images/arrowR.gif';
		...
		if($this->page < $this->max_pages)
		{
		return '<a class="'.$class.'" id="next" href="#'.$this->php_self.'?page='.($this->page+1).'&'.$this->append.'" class="txtred pR5">'.$tag.'</a>'.'</li>';
		}
		
	}


	function renderPrevAjax($tag='', $class = 'previmage') 
	{
		$root="http://".$_SERVER['HTTP_HOST'];
		$URL_SITE = $root;
		$imgL='/images/arrowL.gif';
		...
		if($this->page > 1)
		{
		return '<a id="prev" class="'.$class.'" href="#'.$this->php_self.'?page='.($this->page-1).'&'.$this->append.'" class="txtred pR5">'.$tag.'</a>';
		}

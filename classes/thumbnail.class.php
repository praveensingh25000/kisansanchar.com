<?php 
//File is added to create thumbnails of images.
/**
 * thumbnail.inc.php
 * 
...
class Thumbnail {
    ...
    function showErrorImage() {
		...
		$imgname = "http://maduhaa.com/images/nopic1.JPG";
		//print_r($errImg);
        //$errImg = ImageCreate(80,60);
       // $bgColor = imagecolorallocate($errImg,255,0,0);
       // $fgColor1 = imagecolorallocate($errImg,255,255,255);
       // $fgColor2 = imagecolorallocate($errImg,255,0,0);
       $errImg = @imagecreatefromjpeg($imgname);
	  // resize($errImg,80,60)
        //imagestring($errImg,3,6,6,$this->errmsg,$fgColor1);
        imagepng($errImg);
        imagedestroy($errImg);
    }
    ...
    function resize($maxWidth = 0, $maxHeight = 0) {
		...
		if(function_exists("ImageCreateTrueColor")) {
			$this->workingImage = ImageCreateTrueColor($this->newDimensions['newWidth']-1,$this->newDimensions['newHeight']-1);
		}
		else {
			$this->workingImage = ImageCreate($this->newDimensions['newWidth'],$this->newDimensions['newHeight']);
		}
		    
		 if(!imagealphablending($this->workingImage,FALSE)){
			return FALSE;
		}
		if(!imagesavealpha($this->workingImage,TRUE)){
			return FALSE;
		}
		ImageCopyResampled(
			$this->workingImage,
	...
	function resize_png_image(){
	  ...
	  if(!imagealphablending($this->workingImage,FALSE)){
		...
		if(!imagecopyresampled($this->workingImage,$this->oldImage,0,0,0,0,$destWidth,$destHeight,$srcWidth,$srcHeight)){
			return FALSE;
		}
		return $this->workingImage;
		...
		$srcImage=imagecreatefrompng($watermark);
		if($srcImage==''){
			return FALSE;
		}
		$newWidth = 50;
	...
	function resizewatermark(){
	  /* $watermark = "C:/wamp/www/gallery/upload/w_mark.png";
	   echo*/ $basedir=$_SERVER['DOCUMENT_ROOT']."/gallery/upload/";
	...
	function showwatermark($quality=100,$name = '') {
	  $basedir=$_SERVER['DOCUMENT_ROOT']."/gallery/upload";
		$watermarkimage="w_mark.png";
		 $watermark = $basedir."/".$watermarkimage;

      $waterthumb = new Thumbnail($watermark);
	   $im =  $waterthumb->resizewatermark() ;
	    imagecopyresampled(
            $this->newImage,
            $im,
           (imagesx($this->newImage)/2)-(imagesx($im)/2),
            (imagesy($this->newImage))-(imagesy($im)),
            0,
            0,
            imagesx($im),
            imagesy($im),
	...
	function createReflection($percent,$reflection,$white,$border = true,$borderColor = '#a4a4a4') {
        ...
        $colorToPaint = ImageColorAllocateAlpha($this->workingImage,255,255,255);

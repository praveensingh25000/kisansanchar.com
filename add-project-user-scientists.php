...
$filterassigneduserJson  = $projectObj->getProjectAssociatedGloblaData($tablename='projects_locations',IS_PROJECT_ID,$contentType='s',$type='by_name');
//echo '<pre>';print_r($filterassigneduserArray);echo '</pre>';die;
?>
			...
			<td class="wdthpercent50">
				<form id="select_content_type_form" method="post" action="">
					<select onchange="javascript: FunctionSelectContentType('select_content_type_form');" class="wdthpercent90" id="user_addition_type" name="user_addition_type">
						...
						<option <?php if(isset($_SESSION['user_addition_type']) && $_SESSION['user_addition_type']=='individually'){ echo 'selected="selected"';}?> value="individually">Existing Scientist from KS Database</option>
						<!-- <option <?php if(isset($_SESSION['user_addition_type']) && $_SESSION['user_addition_type']=='grouply'){ echo 'selected="selected"';}?> value="grouply">New Scientist in project</option> -->
						<option <?php if(isset($_SESSION['user_addition_type']) && $_SESSION['user_addition_type']=='importcsv'){ echo 'selected="selected"';}?> value="importcsv">Import CSV Scientist Data</option>
					</select>
				</form>
			</td>					
		</tr>
		...
		<table class="wdthpercent100">
			...
			<tr>
				...
				<td class="wdthpercent50">
					...
					<span class="font11"><a href="<?php echo URL_SITE;?>/images/users-format.csv">Download CSV Format</a></span>
				</td>
			</tr>
			...
			<tr>
				...
				<td class="wdthpercent50">
					 <span class="">
						<input type="submit" class="button" value="<?php echo $langVariables['form_var']['submit']?>" name="uploadprojectassignedscientistcsv">
						<input type="hidden" value="<?php echo $project_id;?>" name="project_id">
						<input type="hidden" value="2" name="project_user_type">
					</span>
					<span class="pL40">
	...
	<form action="action.php" method="post" id="addprojectforms" name="addprojectforms">	
		<table class="wdthpercent100">
			...
			<tr>
				...
				<td class="wdthpercent50">
					 <span class="">
						<input type="submit" class="button" value="<?php echo $langVariables['form_var']['submit']?>" name="submit">
						<input type="hidden" value="<?php echo $project_id;?>" name="project_id">
						<input type="hidden" value="2" name="project_user_type">
						<input type="hidden" value="Scientist" name="uploadprojectassignedscientist">
					</span>
					<span class="pL40">
						<input type="reset" class="button" value="<?php echo $langVariables['form_var']['reset']?>" id="reset">
					</span>

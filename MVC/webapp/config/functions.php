...
function urlsegment($segmentnumber=''){
	$rawData = explode('=',$_SERVER['REQUEST_URI']);
	if(isset($rawData[1])){
		$urlData = explode('/',$rawData[1]);
		return (!empty($urlData[$segmentnumber]))?trim($urlData[$segmentnumber]):$urlData[1];
	}else{
		return '404';
	}
}
...
function getExploded($datastr,$delemeter,$returnpart=null){
	if($returnpart==null){
		...
		return (!empty($dataArray[$returnpart]))?trim($dataArray[$returnpart]):'0';
	}
}
...
function randomDigits($startlen='29',$endlen='51'){
	$digits = $digitsoptionStr = '';
	$digitsoption = array();
	for($i=$startlen;$i<=$endlen;$i++){
	  $digitsoption[] = $i;	   
	}
	return $digit = $digitsoption[array_rand($digitsoption)];
}
...
function checkDashboard($dashbordType=null){
	if(isset($dashbordType) && is_numeric($dashbordType) && $dashbordType!='4'){
...
function checkProjectDashboard($is_project = false){
	if($is_project){
...
function getSessionData($is_admin = false){	
	if($is_admin){
		if(isset($_SESSION['admin_session'])){
		...
		if(isset($_SESSION['session_user_data'])){
			return $_SESSION['session_user_data'];
		}
	}	
}

function getSession($sessionkey=NULL){	
	if(isset($_SESSION['admin_session'])){
		return $_SESSION['admin_session'][$sessionkey];
	}	
}
...
function uploadAllTypeFiles($folder, $formdata, $contentid = null) {
	...
	// list of permitted file types, this is only images but documents can be added  
	$permitted = array('image/gif', 'image/jpeg', 'image/jpg', 'image/pjpeg', 'audio/mp3', 'audio/wav', 'audio/x-wav', 'audio/mpeg', 'video/mp4', 'application/octet-stream', 'application/msword', 'text/csv','application/pdf','application/vnd.ms-excel','application/csv','text/comma-separated-values');
...
function show_date_format($date,$deletemeter=NULL){
	if($deletemeter){
		return date('Y'.$deletemeter.'m'.$deletemeter.'d',strtotime($date));
	}else{
		return date('d M Y',strtotime($date));
	}
}
...
function format_ivr_array_cdr($arr){
	$formatedarray = array();
	if(!empty($arr)){
		foreach($arr as $key=>$arrAll){
			foreach($arrAll as $arrAllone){
				$formatedarray[] = $arrAllone;						
			}
		}
	}
...
function nearestNumber($number=NULL,$digit='0'){
	if($number < 0){
		return '-';
	}else{
...
function getSearchCondition($postedArray,$coloumname,$sqlkeyword,$othercondition=NULL){
	@extract($postedArray);
	$cisearchCondition = '';
	if(isset($cisearch)){
		$_SESSION['cisearch'] = $cisearch;
		$cisearchCondition = " and `".$coloumname."` ".$sqlkeyword." '%".$_SESSION['cisearch']."%' ".$othercondition." ";
	}else if(isset($_SESSION['cisearch'])){
		$cisearch = $_SESSION['cisearch'];
		$cisearchCondition = " and `".$coloumname."` ".$sqlkeyword." '%".$_SESSION['cisearch']."%' ".$othercondition." ";
	}

	return $cisearchCondition;
}

function printdr($arr){
...
function getfarmerName($userDetail,$associated=null){
	if(!empty($userDetail['farmer_name'])){ 
		$fullname = $userDetail['farmer_name']; 
	} else if(!empty($userDetail['farmer_name'])){ 
		$fullname = $userDetail['farmer_name'].' '.$userDetail['plastname']; 
	} else if(!empty($userDetail['username'])){
		$kvkvar = substr($userDetail['username'],0,3);
		if($kvkvar=='kvk'){
			$fullname = ucwords(substr($userDetail['username'],0,3).' '.substr($userDetail['username'],3));
...
function getTotalCount($name){
	$dataArray = $dataArraymain = array();
	$dataArray = explode('(',trim($name));
	if(isset($dataArray[1])){
		$dataArraymain = explode(')',trim($dataArray[1]));
	}
	return (isset($dataArraymain[0]))?trim($dataArraymain[0]):'0';
}

function functionConcateUniversalData($array){
	$insertStr = $insertData = '';
	if(!empty($array)){
		foreach($array as $coloumname => $value){
			if($coloumname!= 'filterDownload'){
				if(is_array($value)){
					$insertStr.= " AND fl.".$coloumname ." IN (".implode(',',$value).") ";
				}else{
					if(!empty($value)){
					$insertStr.= " AND fl.".$coloumname ." = '".mysql_real_escape_string($value)."' ";
					}
				}
			}
		}

...
if(isset($_POST['submitusersetting'])){
	...
	$result     = $adminObj->addUserUserTypeSetting($_POST);		
	$_SESSION['msgsuccess'] = "7";
	header("location: usertype-setting.php");
	exit;	
}
	...
	<div class="entry">
		<form action="" method="post" id="userprivicysettingform" name="userprivicysettingform">
			...
			<div class="wdthpercent100 pT10 pB10">
				<h3 class="title">Select User Type Conent</h3>	
				<div class="tabnav pT5 pB5 pL20">
					<input type="checkbox" id="user-type-check-all">&nbsp;&nbsp;All						
				</div>
				<?php foreach($userTypes as $userTypes) { ?>
					<div class="wdthpercent100 pT5 pB5 pL20">								   
						<input <?php if(!empty($user_type_setting_checkbox) && in_array($userTypes['id'],$user_type_setting_checkbox)){ echo 'checked="checked"';}?> value="<?php echo $userTypes['id'];?>" type="checkbox" id="user_type_setting" name="user_type_setting[]" class="required"/>
						<span class="pL10"><?php echo ucwords($userTypes['user_type']);?></span><br />
						<label style="display:none;" for="user_type_setting" generated="true" class="error">* required.</label>
					</div>

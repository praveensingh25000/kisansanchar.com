...
if(!empty($cdrvoiceData)){
	...
	$sqlordinates  = "SELECT mo.*,m.*,m.date as mdate FROM `message_ordinates_en` mo JOIN `message_en` m ON mo.msg_id = m.id WHERE DATE_FORMAT(mo.date,'%Y-%m-%d') = '".$sendDate."' and m.status='5' and `message_file` != '' ";
		...
		if(!empty($cdrMessageData)){
			foreach($cdrMessageData as $districtkey => $messageAll){
				foreach($messageAll as $phonekey => $message){
					...
					$sql = "INSERT into `message_en` SET
			...
			if(!empty($messageidArray)){
				foreach($messageidArray as $msg_id){
					//UPDATING MSG STATUS
					$sql	= "UPDATE `message_en` SET `status`='6' WHERE `id` = '".$msg_id."' ";
					$update = $db->update($sql);
				}
			}
			if(isset($insertResult) && $insertResult){
				echo 'Message inserted of date(English):'.$sendDate;exit;
			}
		}
	}else{
		echo 'No Message posted date(English): :'.$sendDate;exit;
	}
}else{
	echo 'No Message posted date(English): :'.$sendDate;exit;
}
